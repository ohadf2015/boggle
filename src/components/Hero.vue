...
<main class='flex h-full flex-row items-start justify-evenly bg-gradient-to-tr from-purple-400 to-blue-200'>

<div class='dashboard w-2/12 flex flex-col items-center justify-center '> 
...
<Timer :isStarted=isStarted :timer="timer" @timer-end='endGame'/>
<form  v-if="!isStarted" class="w-full max-w-sm mt-10 mb-2">
  <div class="flex items-center border-b-2 border-indigo-400 py-2">
    <input v-model="timer" type="number" min="1" max="10" class="appearance-none bg-transparent text-center text-8xl font-bold border-none w-full text-black-400 mr-3 py-1 px-2 leading-tight focus:outline-none text-indigo-600"  placeholder="Minutes" aria-label="Full name">
  </div>
</form>
